...
class DoricaHome extends StatefulWidget {
  @override
  _MyHomePageState createState() => _MyHomePageState();
}

class _MyHomePageState extends State<DoricaHome> {
...
return Container(
  ...
  void _loadSongs () async {
     final songz = await DatabaseHelper().getFruits_dor();
     songDesc = await DatabaseHelper().getAllSongs_dor();
     final infoz = await DatabaseHelper().getSongInfos_dor();
     setState(() {
      songs = songz;
      info = infoz;
      });
          ...
          child:  Container(
            decoration: 
                BoxDecoration(
                  ...
                  LinearGradient(
                    colors:
                    [const Color.fromARGB(255, 128, 0, 0), Colors.blue, ],
                    begin: 
                    Alignment.topCenter, 
                    end: 
                    Alignment.bottomCenter,
              ...
              onPressed: () {
                timeDilation = 2.0; // slows down animations speed 
                 _fabVisible? Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DonateScreen(choir_Id: 5,))): null;},
              //color:   Colors.white,
            ),
            SizedBox(width: 10.0,),
            IconButton(
                onPressed: (){
                  _fabVisible? Navigator.push(context,
                    MaterialPageRoute(builder:(context)=>AddSong(fruits: songs, choir_Id: 5,))): null;
                    },
              icon: Icon(Icons.add),
              color: Colors.white,
              ),
  ...
  final TextStyle textStyleNull = TextStyle(
    fontSize: 16, fontFamily: 'VarelaRound', fontWeight: FontWeight.bold, color: const Color.fromARGB(255, 128, 0, 0)
  );
