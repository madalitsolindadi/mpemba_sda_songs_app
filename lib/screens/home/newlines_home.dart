...
class NewlinesHome extends StatefulWidget {

  const NewlinesHome({super.key});
  @override
  _GeneralHomeState createState() => _GeneralHomeState();
}

class _GeneralHomeState extends State<NewlinesHome> {
...
return Container(
            color:  const Color.fromARGB(255, 161, 40, 183), 
            child: Row(
  ...
  void _loadSongs () async {
     ...
     final songz = await DatabaseHelper().getFruits_new();
     songDesc = await DatabaseHelper().getAllSongs_new();
     final infoz = await DatabaseHelper().getSongInfos_new();
  ...
  Widget build(BuildContext context) {
   ...
   return WillPopScope(
    ...
    child: Scaffold(
      ...
      appBar: AppBar(
        //centerTitle: true,
        backgroundColor:   const Color.fromARGB(255, 161, 40, 183), 
              ...
              onPressed: () {
                timeDilation = 2.0; // slows down animations speed 
                 _fabVisible? Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DonateScreen(choir_Id: 10,))): null;},
              //color:   Colors.white,
            ),
            SizedBox(width: 10.0,),
            IconButton(
                onPressed: (){
                  _fabVisible? Navigator.push(context,
                    MaterialPageRoute(builder:(context)=>AddSong(fruits: songs, choir_Id: 10,))): null;
                    },
              icon: Icon(Icons.add),
              color: Colors.white,
              ),
