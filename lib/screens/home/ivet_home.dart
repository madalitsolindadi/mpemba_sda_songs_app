...
class IvetHome extends StatefulWidget {

  const IvetHome({super.key});
  @override
  _GeneralHomeState createState() => _GeneralHomeState();
}

class _GeneralHomeState extends State<IvetHome> {
...
return Container(
  ...
  void _loadSongs () async {
     ...
     final songz = await DatabaseHelper().getFruits_ivet();
     songDesc = await DatabaseHelper().getAllSongs_ivet();
     final infoz = await DatabaseHelper().getSongInfos_ivet();
     final ctitle = 'iVet';
  ...
  Widget build(BuildContext context) {
   ...
   return WillPopScope(
    ...
    child: Scaffold(
      appBar: AppBar(
        //centerTitle: true,
        backgroundColor:  ColorsFile().iveColor(), 
              ...
              onPressed: () {
                timeDilation = 2.0; // slows down animations speed 
                 _fabVisible? Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DonateScreen(choir_Id: 8,))): null;},
              //color:   Colors.white,
            ),
            SizedBox(width: 10.0,),
            IconButton(
                onPressed: (){
                  _fabVisible? Navigator.push(context,
                    MaterialPageRoute(builder:(context)=>AddSong(fruits: songs, choir_Id: 8,))): null;
                    },
              icon: Icon(Icons.add),
              color: Colors.white,
              ),
