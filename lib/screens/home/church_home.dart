...
class ChurchHome extends StatefulWidget {
  ...
  const ChurchHome({super.key});
  @override
  _ChurchHomeState createState() => _ChurchHomeState();
}

class _ChurchHomeState extends State<ChurchHome> {
...
return Container(
  ...
  void _loadSongs () async {
     ...
     final songz = await DatabaseHelper().getFruits_chur();
     songDesc = await DatabaseHelper().getAllSongs_chur();
     final infoz = await DatabaseHelper().getSongInfos_chur();
              ...
              onPressed: () {
                timeDilation = 2.0; // slows down animations speed 
                 _fabVisible? Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DonateScreen(choir_Id: 3,))): null;},
              //color:   Colors.white,
            ),
            SizedBox(width: 10.0,),
            IconButton(
                onPressed: (){
                  _fabVisible? Navigator.push(context,
                    MaterialPageRoute(builder:(context)=>AddSong(fruits: songs, choir_Id: 3,))): null;
                    },
              icon: Icon(Icons.add),
              color: Colors.white,
              ),
            ...
            children: [
              ...
              onTap: () {
                ...
                if (selectedNumber != null && selectedNumber >= 0
                    && selectedNumber <= songs.length-1 && selectedNumber != 0.1
