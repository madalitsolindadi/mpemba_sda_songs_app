...
class SenioryouthHome extends StatefulWidget {

  const SenioryouthHome({super.key});
  @override
  _GeneralHomeState createState() => _GeneralHomeState();
}

class _GeneralHomeState extends State<SenioryouthHome> {
...
return Container(
  ...
  void _loadSongs () async {
     ...
     final songz = await DatabaseHelper().getFruits_senior();
     songDesc = await DatabaseHelper().getAllSongs_senior();
     final infoz = await DatabaseHelper().getSongInfos_senior();
  ...
  Widget build(BuildContext context) {
   ...
   return WillPopScope(
    ...
    child: Scaffold(
      appBar: AppBar(
        //centerTitle: true,
        backgroundColor:  ColorsFile().senColor(),
              ...
              onPressed: () {
                timeDilation = 2.0; // slows down animations speed 
                 _fabVisible? Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DonateScreen(choir_Id: 12,))): null;},
              //color:   Colors.white,
            ),
            SizedBox(width: 10.0,),
            IconButton(
                onPressed: (){
                  _fabVisible? Navigator.push(context,
                    MaterialPageRoute(builder:(context)=>AddSong(fruits: songs, choir_Id: 12,))): null;
                    },
              icon: Icon(Icons.add),
              color: Colors.white,
              ),
