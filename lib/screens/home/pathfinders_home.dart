...
import '../../publisher_info_screen.dart';
import '../add_song.dart';
import '../fruit_detail_view.dart';

class PathfindersHome extends StatefulWidget {
  @override
  _MyHomePageState createState() => _MyHomePageState();
}

class _MyHomePageState extends State<PathfindersHome> {
...
return Container(
  ...
  void _loadSongs () async {
     final songz = await DatabaseHelper().getFruits_path();
     songDesc = await DatabaseHelper().getAllSongs_path();
     final infoz = await DatabaseHelper().getSongInfos_path();
     setState(() {
      songs = songz;
      info = infoz;
      });
   ...
   return WillPopScope(
    ...
    child: Scaffold(
            ...
            flexibleSpace: 
              Container(
                ...
                BoxDecoration(
                  ...
                  LinearGradient(
                    colors:
                    [const Color.fromARGB(255, 238, 205, 38),  const Color.fromARGB(255, 46, 105, 48),],
                    begin: 
                    Alignment.topCenter, 
                    end: 
      ...
      duration: Duration(milliseconds: 300),
          child:  Container(
            decoration: 
                BoxDecoration(
                  ...
                  LinearGradient(
                    colors:
                    [const Color.fromARGB(255, 46, 105, 48), const Color.fromARGB(255, 238, 205, 38), ],
                    begin: 
                    Alignment.topCenter, 
                    end: 
                    Alignment.bottomCenter,
              ...
              onPressed: () {
                timeDilation = 2.0; // slows down animations speed 
                 _fabVisible? Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DonateScreen(choir_Id: 11,))): null;},
              //color:   Colors.white,
            ),
            SizedBox(width: 10.0,),
            IconButton(
                onPressed: (){
                  _fabVisible? Navigator.push(context,
                    MaterialPageRoute(builder:(context)=>AddSong(fruits: songs, choir_Id: 11,))): null;
                    },
              icon: Icon(Icons.add),
              color: Colors.white,
              ),
        ...
        future: DatabaseHelper().getFruits(),
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final title = songs[index]['name'].toString().toLowerCase();
              
              // Check if the search query matches either title or num
              if (title.contains(searchQuery) ||
              getContentNumber(fruits[index]['id']-1).toString().contains(searchQuery)
              || songDesc[index].toString().contains(searchQuery)
              ) {
                return ListTile(
                  title: Text(getContentNumber(songs[index]['id']-1) +'- '+ songs[index]['name'],
                  style: const TextStyle(
                                fontSize: 16.0,
                                fontWeight: FontWeight.bold,
                  ...
                  onTap: () async {
                    // Navigate to the details screen with the specific fruit data
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => FruitDetailView(
                          fruitId: fruits[index]['id'],
                          fruits: songs,
                          initialIndex: fruits[index]['id']-1,
                          description: songDesc,
                        ),
                      ),
                    );
                    //await DatabaseHelper().getFruits();
                  },
                );
              } else {
  ...
  final TextStyle textStyleNull = TextStyle(
    fontSize: 16, fontFamily: 'VarelaRound', fontWeight: FontWeight.bold, color:  const Color.fromARGB(255, 46, 105, 48)
  );
...
void _showNumPadExp(BuildContext context) {
      TextEditingController numberController = TextEditingController();
       String? errorMessage; // To store error message
      final ButtonStyle buttonStyle = TextButton.styleFrom(
    shape: RoundedRectangleBorder(
              ...
              TextField(
               ...
               // blue hint text color
                ...
                prefix: IconButton(
                  ...
                  color: Colors.white,
                  onPressed: () {Navigator.of(context).pop(); _showNumPad(context); },
                  ),
                suffixIcon: IconButton(
                  icon: Icon(Icons.clear),
                  color: Colors.white,
