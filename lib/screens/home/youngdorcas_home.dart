...
class YoungdorcasHome extends StatefulWidget {
  @override
  _MyHomePageState createState() => _MyHomePageState();
}

class _MyHomePageState extends State<YoungdorcasHome> {
...
return Container(
  ...
  void _loadSongs () async {
     final songz = await DatabaseHelper().getFruits_young();
     songDesc = await DatabaseHelper().getAllSongs_young();
     final infoz = await DatabaseHelper().getSongInfos_young();
     setState(() {
      songs = songz;
      info = infoz;
      });
              ...
              onPressed: () {
                timeDilation = 2.0; // slows down animations speed 
                 _fabVisible? Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DonateScreen(choir_Id: 16,))): null;},
              //color:   Colors.white,
            ),
            SizedBox(width: 10.0,),
            IconButton(
                onPressed: (){
                  _fabVisible? Navigator.push(context,
                    MaterialPageRoute(builder:(context)=>AddSong(fruits: songs, choir_Id: 16,))): null;
                    },
              icon: Icon(Icons.add),
              color: Colors.white,
              ),
