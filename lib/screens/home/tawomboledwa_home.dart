...
import '../../options/colors_file.dart';
import '../add_song.dart';
import '../fruit_detail_view.dart';
import '../../options/switch_book.dart';

class TawomboledwaHome extends StatefulWidget {

  const TawomboledwaHome({super.key});
  @override
  _GeneralHomeState createState() => _GeneralHomeState();
}

class _GeneralHomeState extends State<TawomboledwaHome> {
...
return Container(
  ...
  void _loadSongs () async {
     ...
     final songz = await DatabaseHelper().getFruits_tawo();
     songDesc = await DatabaseHelper().getAllSongs_tawo();
     final infoz = await DatabaseHelper().getSongInfos_tawo();
     final ctitle = 'Mpemba SDASongs';
  ...
  Widget build(BuildContext context) {
   ...
   return WillPopScope(
    ...
    child: Scaffold(
      ...
      appBar: AppBar(
        //centerTitle: true,
        backgroundColor:  ColorsFile().tawoColor(), 
        title: !isSearchVisible ?Text(choir_title,
            style: TextStyle(
              fontSize: 16.0,
              fontFamily: 'MAIAN',
              fontWeight: FontWeight.bold,
              ...
              onPressed: () {
                timeDilation = 2.0; // slows down animations speed 
                 _fabVisible? Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DonateScreen(choir_Id: 14,))): null;},
              //color:   Colors.white,
            ),
            SizedBox(width: 10.0,),
            IconButton(
                onPressed: (){
                  _fabVisible? Navigator.push(context,
                    MaterialPageRoute(builder:(context)=>AddSong(fruits: songs, choir_Id: 14,))): null;
                    },
              icon: Icon(Icons.add),
              color: Colors.white,
              ),
