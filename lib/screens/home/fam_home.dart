...
class _GeneralHomeState extends State<FamHome> {
...
return Container(
  ...
  void _loadSongs () async {
     ...
     final songz = await DatabaseHelper().getFruits_fam();
     songDesc = await DatabaseHelper().getAllSongs_fam();
     final infoz = await DatabaseHelper().getSongInfos_fam();
     final ctitle = 'FAM';
              ...
              onPressed: () {
                timeDilation = 2.0; // slows down animations speed 
                 _fabVisible? Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DonateScreen(choir_Id: 6))): null;},
              //color:   Colors.white,
            ),
            SizedBox(width: 10.0,),
            IconButton(
                onPressed: (){
                  _fabVisible? Navigator.push(context,
                    MaterialPageRoute(builder:(context)=>AddSong(fruits: songs, choir_Id: 6,))): null;
                    },
              icon: Icon(Icons.add),
              color: Colors.white,
              ),
