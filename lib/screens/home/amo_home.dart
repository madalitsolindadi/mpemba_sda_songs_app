...
class AmoHome extends StatefulWidget {

  const AmoHome({super.key});
  @override
  _AmoHomeState createState() => _AmoHomeState();
}

class _AmoHomeState extends State<AmoHome> {
...
return Container(
  ...
  void _loadSongs () async {
     ...
     final songz = await DatabaseHelper().getFruits_amo();
     songDesc = await DatabaseHelper().getAllSongs_amo();
     final infoz = await DatabaseHelper().getSongInfos_amo();
     final ctitle = 'AMO';
              ...
              onPressed: () {
                timeDilation = 2.0; // slows down animations speed 
                 _fabVisible? Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DonateScreen(choir_Id: 1,))): null;},
              //color:   Colors.white,
            ),
            SizedBox(width: 10.0,),
            IconButton(
                onPressed: (){
                  _fabVisible? Navigator.push(context,
                    MaterialPageRoute(builder:(context)=>AddSong(fruits: songs, choir_Id: 1,))): null;
                    },
              icon: Icon(Icons.add),
              color: Colors.white,
              ),
