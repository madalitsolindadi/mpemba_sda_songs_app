...
class ComfortHome extends StatefulWidget {

  const ComfortHome({super.key});
  @override
  _GeneralHomeState createState() => _GeneralHomeState();
}

class _GeneralHomeState extends State<ComfortHome> {
...
return Container(
            color: const Color.fromARGB(255, 75, 14, 97), 
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
               SelectableText(ctitle,
              style: TextStyle(
              fontSize: 18.0,
              fontFamily: 'MAIAN',
  ...
  void _loadSongs () async {
     ...
     final songz = await DatabaseHelper().getFruits_comf();
     songDesc = await DatabaseHelper().getAllSongs_comf();
     final infoz = await DatabaseHelper().getSongInfos_comf();
  ...
  Widget build(BuildContext context) {
   ...
   return WillPopScope(
    ...
    child: Scaffold(
      ...
      appBar: AppBar(
        //centerTitle: true,
        backgroundColor:  const Color.fromARGB(255, 75, 14, 97),
              ...
              onPressed: () {
                timeDilation = 2.0; // slows down animations speed 
                 _fabVisible? Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DonateScreen(choir_Id: 4,))): null;},
              //color:   Colors.white,
            ),
            SizedBox(width: 10.0,),
            IconButton(
                onPressed: (){
                  _fabVisible? Navigator.push(context,
                    MaterialPageRoute(builder:(context)=>AddSong(fruits: songs, choir_Id: 4,))): null;
                    },
              icon: Icon(Icons.add),
              color: Colors.white,
              ),
