...
class TowerHome extends StatefulWidget {

  const TowerHome({super.key});
  @override
  _GeneralHomeState createState() => _GeneralHomeState();
}

class _GeneralHomeState extends State<TowerHome> {
...
return Container(
            color: const Color.fromARGB(255, 46, 105, 102), 
            child: Row(
  ...
  void _loadSongs () async {
     ...
     final songz = await DatabaseHelper().getFruits_tow();
     songDesc = await DatabaseHelper().getAllSongs_tow();
     final infoz = await DatabaseHelper().getSongInfos_tow();
  ...
  Widget build(BuildContext context) {
   ...
   return WillPopScope(
    ...
    child: Scaffold(
      ...
      appBar: AppBar(
        //centerTitle: true,
        backgroundColor:   const Color.fromARGB(255, 46, 105, 102),  
        title: !isSearchVisible ?Text(ch_title,
            style: TextStyle(
              fontSize: 16.0,
              fontFamily: 'MAIAN',
              fontWeight: FontWeight.bold,
              ...
              onPressed: () {
                timeDilation = 2.0; // slows down animations speed 
                 _fabVisible? Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DonateScreen(choir_Id: 15,))): null;},
              //color:   Colors.white,
            ),
            SizedBox(width: 10.0,),
            IconButton(
                onPressed: (){
                  _fabVisible? Navigator.push(context,
                    MaterialPageRoute(builder:(context)=>AddSong(fruits: songs, choir_Id: 15,))): null;
                    },
              icon: Icon(Icons.add),
              color: Colors.white,
              ),
